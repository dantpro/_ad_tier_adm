...
$CSVObject | ForEach-Object {
    ...
    if ($_.$Tier0Header -eq $true) {
        ...
        New-ADUser -Path "OU=Users,OU=Tier 0,OU=Administration,OU=Corp,DC=contoso,DC=lab" -Name $Username -Enabled $false
    ...
    if ($_.$Tier1Header -eq $true) {
        ...
        New-ADUser -Path "OU=Users,OU=Tier 1,OU=Administration,OU=Corp,DC=contoso,DC=lab" -Name $Username -Enabled $false
        Write-Host "Created user $Username"
    ...
    if ($_.$Tier2Header -eq $true) {
        ...
        New-ADUser -Path "OU=Users,OU=Tier 2,OU=Administration,OU=Corp,DC=contoso,DC=lab" -Name $Username -Enabled $false
        Write-Host "Created user $Username"
